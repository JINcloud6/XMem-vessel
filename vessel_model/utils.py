...
def compute_attention_entropy(attn_weights, mask, sample_points=1024, eps=1e-6):
    """
    ...
    if isinstance(mask, np.ndarray):
        mask_tensor = torch.from_numpy(mask).to(attn_weights.device)
    else:
        mask_tensor = mask.to(attn_weights.device)
